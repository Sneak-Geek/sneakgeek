...
export class Migration_07272021 extends BaseMigration {
  private defaultSneakGeekUrl = "https://storage.googleapis.com/sneakgeek-images/sneakgeek-default-image.png";
  private stockxUrlRegExp = /placeholder/i;
  constructor(
    @inject(Types.ShoeRepository) private shoeRepo: Repository<Shoe>
  ) {
    super();
  }
  ...
  public async run() {
    try {
      const defaultStockxImage = await this.shoeRepo.find({
      ...
      if (defaultStockxImage.length > 0) {
        ...
        await this.shoeRepo.updateMany(
          { "media.thumbUrl": this.stockxUrlRegExp },
          {
            "$set": {
              "media": {
